...
> When the connection state, display state, acquisition state, and stimulus indicator light are all on, click to start recording data and click Pause to end recording, and the data will be automatically saved as 3 independent files. You can download the **UI_Collector.exe** tool in ![Release](https://github.com/transover/sEMG-MMG_SYNC/releases/UI_Collector) for the task of sEMG-MMG acquisition, as shown in the following figure:
